...
import (
	...
	"github.com/alist-org/alist/v3/server/handles/sdk"
...
func SecondValidate(c *gin.Context) {
	...
	geetestSeccode := c.PostForm("geetest_seccode")
	geetestValidate := c.PostForm("geetest_validate")
	geetestChallenge := c.PostForm("geetest_challenge")
	fmt.Printf("Received geetest_seccode: %s\n", geetestSeccode)
	fmt.Printf("Received geetest_validate: %s\n", geetestValidate)
	fmt.Printf("Received geetest_challenge: %s\n", geetestChallenge)
	// bypassStatus := GetBypassCache()
	var result *sdk.GeetestLibResult
	...
	if result.Status == 1 {
		...
		common.ErrorStrResp(c, geetestSeccode+geetestValidate+geetestChallenge, 400)
	}
}
